    ...
    pub fn start_guest(
        ...
        let (scaffolding_port, host_ip) = 'local_port: {
            for _ in 0..5 {
                ...
                *difficulty = crate::easytier::calc_conn_difficulty(local_nat, server_nat);
                logging!(
                    "RoomExperiment",
        ...
        thread::spawn(move || {
            loop {
...

                {
                    ...
                    for item in serde_json::from_slice::<serde_json::Value>(data.as_slice()).ok()?.as_array()? {
                    ...
                    for i in (0..profiles.len()).rev() {
                        ...
                        match profile.get_kind() {
                            ProfileKind::HOST => match server_profiles
                                .binary_search_by_key(&profile.get_machine_id(), |p| {
                                    p.get_machine_id()
                                }) {
                                Ok(index)
                                    if !used[index]
